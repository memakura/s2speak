...
def Mecab_initialize(logwrite_ = None, mecab_dir = None):
	...
	if libmc is None:
		...
		libmc.mecab_new.restype = mecab_node_t_ptr
	global mecab
	if mecab is None:
		...
		try:
			...
			s = f.read().strip()
			f.close()
			logwrite_('mecab:' + libmc.mecab_version().decode() + ' ' + s)
		...
		args = (c_char_p * 5)(b'mecab', b'-d', dic.encode('utf-8'), b'-r', mecabrc.encode('utf-8'))
